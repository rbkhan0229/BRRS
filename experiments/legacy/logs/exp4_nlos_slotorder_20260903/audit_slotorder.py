#!/usr/bin/env python3
"""Read-only audit of M32 234/324 owner-order crossover; preserve PER failures."""
import csv
import hashlib
...
def audit(mode, run):
    ...
    sb = 2000
    preamble = 32
    sequence = "324" if mode == "324" else None
    seq_suffix = "_seq324" if sequence else ""
    folder = ROOT.parent / f"exp4_nlos_slotorder_full_6.9m_g200_l15_pac8_sb2000_sp2002{seq_suffix}_20260903_spiopt"
    ...
    try:
        for role, serial in SERIALS.items():
            ...
            assert meta["environment"] == "nlos_slotorder_full"
...
if __name__ == "__main__":
    plan = [("234",1), ("324",1), ("234",4)]
    if len(sys.argv) == 3:
        mode, run = sys.argv[1], int(sys.argv[2])
        assert (mode, run) in plan
        plan = [(mode,run)]
    elif len(sys.argv) != 1:
        raise SystemExit("usage: audit_slotorder.py [234|324 1|4]")
