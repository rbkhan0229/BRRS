...
extern void dwt_readcir_48b(uint8_t *buffer, dwt_acc_idx_e acc_idx, uint16_t sample_offs, uint16_t num_samples);
extern void dwt_readcir(uint32_t *buffer, dwt_acc_idx_e cir_idx, uint16_t sample_offs,
...
#define POINT_NUM 80 // point number
#define ACCUM_DATA_LEN POINT_NUM * (3 + 3) + 1 // num * (real bytes + img bytes) + dummy
static uint8_t accum_data[ACCUM_DATA_LEN];

static uint32_t cir_buffer[POINT_NUM * 2 + 1]; // for cir data, custome buffer
...
int rx_diagnostics(void)
{
    ...
    while (1)
    {
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK)
        {
            ...
            uint16_t fp_int = rx_diag.ipatovFpIndex >> 6; // integer index
            uint16_t offset = (fp_int > 2) ? (fp_int - 2) : 0;
            ...
            dwt_readcir(cir_buffer, DWT_ACC_IDX_IP_M, offset, POINT_NUM, DWT_CIR_READ_LO);
            
            printf("Read %d points from 1,016 points \r\n", POINT_NUM);
            
            int i;
            for (i = 0; i < POINT_NUM; i++) {
                int16_t real = (int16_t)(cir_buffer[i] & 0xFFFF);
                int16_t imag = (int16_t)((cir_buffer[i] >> 16) & 0xFFFF);
                float magnitude = sqrtf((float)(real * real + imag * imag));

                //printf("%d: Sample = %d + %dj\t| Magnitude = %.2f\n", i + 1, real, imag, magnitude);
                printf("%d: %.2f\n", i + 1, magnitude);
            }
