...
ap=argparse.ArgumentParser();ap.add_argument('variant',);ap.add_argument('run',type=int);a=ap.parse_args()
...
try:
    ...
    for side,roles in [('init',['init']),('tx',['N2','N3','N4'])]:
        ...
        for role in roles:
            ...
            if role=='init':v.verify_init(lines,32,3,250,m['parameters_by_variant'][a.variant]['lead_us'],m['parameters_by_variant'][a.variant]['PAC'])
            else:
    ...
    expected_mode='per_slot_delayed_bounded_single_attempt'
    ...
    assert spi['status']=='PASS'
    for key in ['active','begin_fail','end_fail','device_id_fail','state_error','transfer_error','direct_timeout','recovery']:
        assert int(spi[key])==0,(key,spi[key])
    if m['parameters_by_variant'][a.variant].get('SPI_OPT',0):
        assert int(spi['begin'])==1000 and int(spi['end'])==1000 and int(spi['direct_xfers'])>0,spi
        assert cfg['spi_mode']=='persistent_data_burst' and spi['mode']=='persistent_data_burst',cfg
    else:
        assert spi['mode']=='legacy_per_transaction' and int(spi['begin'])==0 and int(spi['end'])==0,spi
    ...
    if True:
        ...
        for w in windows:
            ...
            window_us=97+m['parameters_by_variant'][a.variant]['lead_us']
            assert int(w['window_us'])==window_us and int(w['fwto_uus'])==(window_us*10000+10255)//10256,w
