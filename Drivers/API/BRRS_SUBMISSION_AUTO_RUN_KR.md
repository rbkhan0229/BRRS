...
Exp4는 실제 연결한 센서 수를 `--sensors`로 지정해야 한다. 예를 들어 S3는 N2,
N3, N4와 INIT의 네 보드에서 각각 한 명령을 실행한다. 센서 세 명령을 먼저 시작하고
INIT을 마지막에 시작한다.

```bash
./brrs_run_experiment.sh exp4 N2 iron_door_nlos 6.9 --sensors 3 --serial 1050273888
./brrs_run_experiment.sh exp4 N3 iron_door_nlos 6.9 --sensors 3 --serial 1050211584
./brrs_run_experiment.sh exp4 N4 iron_door_nlos 6.9 --sensors 3 --serial 1050283069
./brrs_run_experiment.sh exp4 init iron_door_nlos 6.9 --sensors 3
```

각 역할은 기본적으로 M=32/64/256을 한 번씩, 총 3개 케이스로 수행한다. 논문용
10회 반복은 모든 역할 명령에 `--repeats 10`을 붙인다. 기본 guard는 200 us이다.
연결하지 않은 노드 역할이나 S3에서 N5처럼 범위를 벗어난 역할은 실행
전에 거부한다.
