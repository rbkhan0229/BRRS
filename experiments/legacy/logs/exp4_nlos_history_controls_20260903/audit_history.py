#!/usr/bin/env python3
"""Read-only audit of M32 receiver-history controls; preserve PER failures."""
import csv
import hashlib
...
def audit(mode, run):
    fast = False
    cases = {"base": (200, 2000, None, True), "g400": (400, 2000, None, True), "first2297": (200, 2297, "324", True), "legacy": (400, 2000, None, False)}
    guard, sb, sequence, spi_opt = cases[mode]
    preamble = 32
    seq_suffix = f"_seq{sequence}" if sequence else ""
    spi_suffix = "_spiopt" if spi_opt else ""
    folder = ROOT.parent / f"exp4_nlos_history_full_6.9m_g{guard}_l15_pac8_sb{sb}_sp2002{seq_suffix}_20260903{spi_suffix}"
    ...
    try:
        for role, serial in SERIALS.items():
            ...
            assert meta["environment"] == "nlos_history_full"
            assert meta["slot_sequence"] == (sequence or "default-round-robin")
            for key, value in {"preamble_symbols":str(preamble), "sensor_count":"3", "guard_us":str(guard), "lead_us":"15", "pac":"8", "sync_buffer_us":str(sb), "sync_prep_us":"2002", "target_cycles":"1000", "rx_event_source":"fint-polling", "phy_fast_skip_pgf":"disabled", "phy_config_profile":"disabled", "rx_path_profile":"disabled", "spim_start_end_profile":"disabled"}.items():
            ...
            if role != "init":
                v.verify_sensor(lines, preamble, 3, int(role[1]), guard, sb, 2002, sequence=sequence, expected_cycles=1000, phy_fast=fast)
        ...
        v.verify_init(logs["init"], preamble, 3, guard, 15, 8, sb, 2002, 5.0, sequence=sequence, spi_opt=spi_opt, expected_cycles=1000, phy_fast=fast)
    ...
    double_buffer = v.key_values(v.last_line(logs["init"], "EXP4_DOUBLE_BUFFER_CSV,"))
    for key in ("rdb_resync", "rdb_incomplete_recovered"):
        v.require(double_buffer, key, 0)
    slot_rows = [v.key_values(line) for line in logs["init"]
                 if line.startswith("EXP4_SLOT_CLASS_CSV,")]
    owner_order = sequence or "234"
    expected_slots = {f"N{node}": slot for slot, node in enumerate(owner_order)}
    assert len(slot_rows) == 3
    assert {row["src"] for row in slot_rows} == set(expected_slots)
    for row in slot_rows:
        source = row["src"]
        assert row["observed_owner"] == source
        assert int(row["observed_slot"]) == expected_slots[source]
        assert int(row["count"]) == nodes[source]["rx"]
        assert int(row["host0"]) + int(row["host1"]) == int(row["count"])
...
if __name__ == "__main__":
    plan = [(m,1) for m in ("g400","first2297","base")] + [("legacy",4), ("g400",4)]
    if len(sys.argv) == 3:
        mode, run = sys.argv[1], int(sys.argv[2])
        assert (mode, run) in plan
        plan = [(mode,run)]
    elif len(sys.argv) != 1:
        raise SystemExit("usage: audit_history.py [g400|first2297|legacy|base 1|4]")
