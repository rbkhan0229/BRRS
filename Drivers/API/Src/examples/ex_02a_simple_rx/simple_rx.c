...
#define APP_NAME "SIMPLE RX Unicast with Source A between B(this host) v1.0" // 20250528 mj
...
/* Sequential preamble code scanning variables (matching TX) */
...
int simple_rx(void)
{
    ...
    current_scan_index = 0; // Start with code 9 (index 0)
    config.rxCode = preamble_codes_64mhz[0]; // Start with first preamble code (9)
    ...
    /* Synchronization delay for TX-RX alignment */
    Sleep(SYNC_DELAY_MS);
    ...
    Sleep(10000);
    test_run_info((unsigned char *)"RX starting sync detection...");
    ...
    while (TRUE)
    {
        ...
        if (!sync_established) {
            /* Wait on code 9 for initial synchronization */
            no_packet_counter++;
            
            /* Keep waiting on code 9 - no automatic switching until sync */
            ...
            if (sync_packet_count >= TX_PACKETS_PER_CODE) {
                ...
                config.rxCode = preamble_codes_64mhz[current_scan_index];
            ...
            if (packets_received_current_code == 0) {
                ...
                if (no_packet_counter >= SYNC_LOSS_THRESHOLD) {
                    /* Lost synchronization - return to code 9 */
                    sync_established = false;
                    current_scan_index = 0;
                    config.rxCode = preamble_codes_64mhz[0];
                    sync_packet_count = 0;
                    packets_received_current_code = 0;
                    no_packet_counter = 0;
                    
                    /* Reconfigure with code 9 using high-precision timing */
                    if (precision_configure(&config)) {
                        test_run_info((unsigned char *)"RX RECONFIG FAILED");
                        while (1) { };
                    }
                    ...
                    test_run_info((unsigned char *)"SYNC LOST - Returning to code 9");
                }
            } else {
                ...
                if (source_id == 'A' && dest_id == device_id) {
                    ...
                    if (config.rxCode != tx_current_code) {
                        ...
                        for (int i = 0; i < num_preamble_codes; i++) {
                            if (preamble_codes_64mhz[i] == tx_current_code) {
        ...
        else if(status_reg & SYS_STATUS_ALL_RX_TO)
        {
            ...
            /* Print preamble code success statistics */
            test_run_info((unsigned char *)"=== RX Preamble Code Stats ===");
