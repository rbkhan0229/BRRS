...
#endif
#if BRRS_OPT_RX_ERROR_DIAG && (BRRS_EXPERIMENT != 4 || BRRS_EXP4_IRQ_PENDING || BRRS_OPT_RX_PATH_PROFILE || BRRS_OPT_PHY_CONFIG_PROFILE || BRRS_OPT_SPIM_START_END_PROFILE)
#error "RX error diagnostics require standalone Exp4 polling without other profiling"
#endif
#if BRRS_OPT_RX_ERROR_DIAG
#include "brrs_rx_error_diag.h"
#endif
 ...
 * buffer whose RXFR, RXFCG and CIADONE flags are already complete. RX is
 * deliberately re-armed only after this wait, so any retry time is included
 * in the measured service time and guard validation. */
...
/* Diagnostic trial: change only the DATA SFD deadline; keep the bounded
 * per-slot RX window, PAC4, beacon PHY and all TX images unchanged. */
#define EXP4_TRIAL_DATA_SFD_TIMEOUT 64U
static uint16_t exp4_trial_sfd_readback = 0U;
static uint32_t exp4_trial_sfd_readback_count = 0U;
static dwt_config_t config_data = {
    9, DATA_PLEN, DATA_PAC,
    9, 9, DATA_SFD_TYPE,
    DWT_BR_6M8, DWT_PHRMODE_STD, DATA_PHR_RATE,
    EXP4_TRIAL_DATA_SFD_TIMEOUT,
...
static uint32_t rx_err_fint_only = 0; /* legacy four-bit fallback; NOT proof of status clearing */
...
static uint32_t exp4_rdb_global_ciadone_fallbacks = 0;
...
typedef struct {
    uint8_t header[IDX_DATA_PAYLOAD];
    uint32_t rx_ts_high32;
    uint32_t rx_open_high32;
    uint16_t frame_length;
    uint8_t header_length;
    uint8_t completed_rx_slot;
    uint8_t observed_rx_slot;
    uint8_t completed_host_buffer;
    bool observed_slot_valid;
    bool rx_open_timing_valid;
} exp4_rx_record_t;

static exp4_rx_record_t exp4_rx_records[BRRS_MAX_DATA_SLOTS];
static uint8_t exp4_rx_record_count = 0U;
...
static latency_stats_t exp4_error_detail_post_rearm_stats;
...
static latency_stats_t exp4_event_to_metadata_stats;
static latency_stats_t exp4_event_to_status_clear_stats;
static latency_stats_t exp4_event_to_header_stats;
static latency_stats_t exp4_hot_path_stats;
...
static void exp4_log_phy_config_profile(void)
{
    ...
    for (target = 0U; target < BRRS_PHY_PROFILE_TARGET_COUNT; target++) {
        snprintf(line, sizeof(line),
                 "EXP4_PHY_CONFIG_STATE_CSV,role=coordinator,target=%s,samples=%lu,idle=%lu,not_idle=%lu",
                 target_names[target],
        ...
        for (phase = 0U; phase < BRRS_PHY_PROFILE_PHASE_COUNT; phase++) {
            ...
            snprintf(line, sizeof(line),
                     "EXP4_PHY_CONFIG_PROFILE_CSV,role=coordinator,target=%s,phase=%s,count=%lu,min_us=%lu,max_us=%lu,avg_x1000_us=%llu",
                     target_names[target], phase_names[phase],
                     (unsigned long)stats->count,
                     (unsigned long)(stats->count ?
...
static void exp4_record_hot_path(uint32_t latency_us)
{
    uint32_t bin = latency_us;

    update_node_latency(&exp4_hot_path_stats, latency_us);
    if (bin >= EXP4_HOT_PATH_HIST_BINS) {
        bin = EXP4_HOT_PATH_HIST_BINS - 1U;
        exp4_hot_path_hist_overflow++;
    }
    exp4_hot_path_hist[bin]++;
}

static uint32_t exp4_hot_path_percentile(uint32_t numerator,
                                         uint32_t denominator)
{
    ...
    if (exp4_hot_path_stats.count == 0U || denominator == 0U) {
        return 0U;
    }
    target = (uint32_t)(((uint64_t)exp4_hot_path_stats.count * numerator +
                         denominator - 1U) / denominator);
    for (bin = 0U; bin < EXP4_HOT_PATH_HIST_BINS; bin++) {
        cumulative += exp4_hot_path_hist[bin];
        if (cumulative >= target) {
            return bin;
        }
    }
...
#if !BRRS_OPT_RX_ERROR_DIAG
static uint32_t exp4_read_error_detail(latency_stats_t *stats)
{
    uint32_t phase_start_cycles = dwt_timer_get_cycles();
    uint32_t status = exp4_spi_read_u32(SYS_STATUS_ID);
    uint32_t phase_us = exp4_cycles_to_us_ceil(
...
#if BRRS_OPT_RX_ERROR_DIAG
static brrs_rxerr_diag_t exp4_error_diag;

static uint32_t exp4_error_diag_read_status(uint32_t *lo, uint16_t *hi)
{
    /* SYS_STATUS occupies six adjacent octets at 00:44..49 (UM p93).
     * Reading is non-clearing. This is not an atomic RF-state snapshot.
     * SPI transport failures still fail the common SPI integrity checks.
     */
    uint8_t bytes[6] = {0};
    uint32_t start = dwt_timer_get_cycles();
    exp4_spi_read_device(SYS_STATUS_ID, 0U, sizeof(bytes), bytes);
    uint32_t cycles = dwt_timer_get_cycles() - start;
    *lo = (uint32_t)bytes[0] | ((uint32_t)bytes[1] << 8U) |
          ((uint32_t)bytes[2] << 16U) | ((uint32_t)bytes[3] << 24U);
    *hi = (uint16_t)bytes[4] | ((uint16_t)bytes[5] << 8U);
    return cycles;
}

static void exp4_error_diag_pre(brrs_rxerr_record_t *record,
                                uint8_t fint, bool timeout)
{
    memset(record, 0, sizeof(*record));
    record->sf = current_cycle;
    record->logical_slot = current_rx_slot;
    record->estimated_slot = 0xFFU;
    record->host = exp4_rx_host_buffer;
    record->poll_fint = fint; /* Original dispatch FINT, not another SPI read. */
    record->timeout = timeout;
    record->pre_cycles = exp4_error_diag_read_status(&record->pre_lo,
                                                    &record->pre_hi);
}

static uint32_t exp4_error_diag_post(brrs_rxerr_record_t *record,
                                    uint8_t estimated_slot,
                                    latency_stats_t *stats)
{
    uint32_t start;
    record->estimated_slot = estimated_slot;
    record->post_cycles = exp4_error_diag_read_status(&record->post_lo,
                                                     &record->post_hi);
    if (stats != NULL) {
        update_node_latency(stats, exp4_cycles_to_us_ceil(record->post_cycles));
    }
    start = dwt_timer_get_cycles();
    exp4_spi_read_device(FINT_STAT_ID, 0U, 1U, &record->post_fint);
    record->fint_cycles = dwt_timer_get_cycles() - start;
    /* Only a bounded struct copy here. Classification/formatting is deferred.
     * Keep the original post-read status for control/legacy counters so the
     * diagnostic flag is not also a new RX recovery algorithm.
     */
    brrs_rxerr_enqueue(&exp4_error_diag, record);
    return record->post_lo;
}

static void exp4_error_diag_print(void)
{
    static const char *const names[BRRS_RXERR_STATUS_BITS] = {
        "IRQS", "CPLOCK", "SPICRCE", "AAT", "TXFRB", "TXPRS", "TXPHS", "TXFRS",
        "RXPRD", "RXSFDD", "CIADONE", "RXPHD", "RXPHE", "RXFR", "RXFCG", "RXFCE",
        "RXFSL", "RXFTO", "CIAERR", "VWARN", "RXOVRR", "RXPTO", "RES22", "SPIRDY",
        "RCINIT", "PLL_HILO", "RXSTO", "HPDWARN", "CPERR", "ARFE", "RES30", "RES31",
        "RES32", "RXPREJ", "RES34", "RES35", "VT_DET", "GPIOIRQ", "AES_DONE", "AES_ERR",
        "CMD_ERR", "SPI_OVF", "SPI_UNF", "SPIERR", "CCA_FAIL", "RES45", "RES46", "RES47"
    };
    static const char *const phases[BRRS_RXERR_TIMINGS] = {
        "pre_status", "post_status", "post_fint", "detect_to_rearm_return"
    };
    static char line[512];
    const brrs_rxerr_diag_t *d = &exp4_error_diag;
    bool valid = d->queue_overflow == 0U && d->queue_count == 0U &&
                 d->processed == d->events && d->hw_faults == 0U &&
                 d->events == total_rx_errors + total_rx_timeouts;
    snprintf(line, sizeof(line),
             "EXP4_RX_ERROR_DIAG_CSV,enabled=1,version=1,events=%lu,errors=%lu,timeouts=%lu,processed=%lu,rearmed=%lu,queue_overflow=%lu,sample_capacity=%u,samples=%lu,samples_omitted=%lu,pre_zero=%lu,post_zero=%lu,changed=%lu,hw_faults=%lu,status=%s",
             (unsigned long)d->events, (unsigned long)d->errors,
             (unsigned long)d->timeouts, (unsigned long)d->processed,
             (unsigned long)d->rearmed, (unsigned long)d->queue_overflow,
             BRRS_RXERR_SAMPLE_CAPACITY, (unsigned long)d->sample_count,
             (unsigned long)d->samples_omitted, (unsigned long)d->pre_zero,
             (unsigned long)d->post_zero, (unsigned long)d->changed,
             (unsigned long)d->hw_faults, valid ? "PASS" : "FAIL");
    final_log_info(line);
    for (uint32_t bit = 0; bit < BRRS_RXERR_STATUS_BITS; bit++) {
        snprintf(line, sizeof(line),
                 "EXP4_RX_ERROR_BIT_CSV,bit=%lu,name=%s,pre_count=%lu,post_count=%lu",
                 (unsigned long)bit, names[bit], (unsigned long)d->pre_bits[bit],
                 (unsigned long)d->post_bits[bit]);
        final_log_info(line);
    }
    for (uint32_t phase = 0; phase < BRRS_RXERR_TIMINGS; phase++) {
        const brrs_rxerr_timing_t *t = &d->timing[phase];
        uint64_t avg = t->count ? t->sum_us * 1000ULL / t->count : 0ULL;
        snprintf(line, sizeof(line),
                 "EXP4_RX_ERROR_TIMING_CSV,phase=%s,count=%lu,min_us=%lu,max_us=%lu,avg_x1000_us=%llu",
                 phases[phase], (unsigned long)t->count, (unsigned long)t->min_us,
                 (unsigned long)t->max_us, (unsigned long long)avg);
        final_log_info(line);
    }
    for (uint32_t i = 0; i < d->sample_count; i++) {
        const brrs_rxerr_record_t *r = &d->samples[i];
        snprintf(line, sizeof(line),
                 "EXP4_RX_ERROR_SAMPLE_CSV,index=%lu,event=%lu,kind=%s,sf=%lu,logical_slot=%u,estimated_slot=%u,host=%u,poll_fint=0x%02X,post_fint=0x%02X,pre_lo=0x%08lX,pre_hi=0x%04X,post_lo=0x%08lX,post_hi=0x%04X,rearmed=%u",
                 (unsigned long)i, (unsigned long)r->event,
                 r->timeout ? "timeout" : "error", (unsigned long)r->sf,
                 (unsigned)r->logical_slot, (unsigned)r->estimated_slot,
                 (unsigned)r->host, (unsigned)r->poll_fint, (unsigned)r->post_fint,
                 (unsigned long)r->pre_lo, (unsigned)r->pre_hi,
                 (unsigned long)r->post_lo, (unsigned)r->post_hi,
                 (unsigned)r->rearmed);
        final_log_info(line);
    }
}
                                   ...
                                   bool profile_good_path)
{
    ...
    if (exp4_has_later_slot()) {
        ...
        uint32_t service_us = exp4_cycles_to_us_ceil(service_cycles);
        update_node_latency(&exp4_rearm_service_stats, service_us);
        if (service_us + RX_LEAD_MARGIN_US > SLOT_GUARD_US) {
            exp4_rearm_deadline_misses++;
        }
        return true;
    }
...
static void exp4_process_deferred_records(void)
{
    uint8_t record_idx;

    if (exp4_rx_record_count == 0U) {
        return;
    }

    for (record_idx = 0U; record_idx < exp4_rx_record_count; record_idx++) {
        const exp4_rx_record_t *record = &exp4_rx_records[record_idx];
        const uint8_t *header = record->header;
        bool frame_length_valid = (record->frame_length == PSDU_BYTES);
        uint8_t src_idx = 0U;

        if (!frame_length_valid) {
            exp4_wrong_length_frames++;
            exp4_wrong_length_by_buffer[record->completed_host_buffer & 1U]++;
            exp4_wrong_length_by_slot[record->observed_slot_valid ?
                record->observed_rx_slot : EXP4_SLOT_CLASS_INVALID]++;
            if (record->frame_length < 128U) {
                exp4_wrong_length_hist[record->frame_length]++;
            }
        }

        if (record->header_length > IDX_SOURCE) {
            src_idx = node_id_to_index(header[IDX_SOURCE]);
        }
        if (frame_length_valid &&
            record->header_length == IDX_DATA_PAYLOAD &&
            header[IDX_MSG_TYPE] == MSG_TYPE_DATA &&
            header[IDX_PROTOCOL_VERSION] == BRRS_PROTOCOL_VERSION &&
            src_idx < TOTAL_ARRAY_SIZE && src_idx != 0U) {
            ...
            uint32_t classified_rx_offset_us = dwt_high32_delta_to_us(
                last_sync_tx_ts_high32, record->rx_ts_high32);

            if (record->observed_slot_valid) {
                expected_owner_seq = current_beacon_config.slot_owner[
                    record->observed_rx_slot];
                expected_slot_start_us =
                    current_beacon_config.first_slot_offset_us +
                    (uint32_t)record->observed_rx_slot *
                    current_beacon_config.slot_interval_us;
                class_slot = record->observed_rx_slot;
            }

            exp4_slot_class_count[src_idx][class_slot]++;
            exp4_slot_class_host_count[src_idx][class_slot]
                                       [record->completed_host_buffer & 1U]++;
            if (!record->observed_slot_valid ||
                src_idx != (uint8_t)(expected_owner_seq - 1U)) {
                exp4_wrong_slot_frames++;
                update_node_latency(&exp4_wrong_slot_rx_offset[src_idx],
                                    classified_rx_offset_us);
                slot_matches = false;
            }
            if (exp4_read_superframe_seq(header) != (uint16_t)current_cycle) {
                exp4_wrong_superframe_frames++;
                slot_matches = false;
            }

            if (slot_matches &&
                !exp4_slot_received[record->observed_rx_slot]) {
                exp4_slot_received[record->observed_rx_slot] = 1U;
            ...
            if (accept_new_frame) {
                ...
                update_node_latency(&node_uwb_rx_offset[src_idx],
                    dwt_high32_delta_to_us(last_sync_tx_ts_high32,
                                           record->rx_ts_high32));
                actual_offset_high32 =
                    record->rx_ts_high32 - last_sync_tx_ts_high32;
                expected_offset_high32 = (uint32_t)
                    (US_TO_DWT_TIME(expected_slot_start_us) >> 8);
                update_signed_timing(&node_slot_timing_error[src_idx],
                    dwt_slot_error_to_ns(actual_offset_high32,
                                         expected_offset_high32));

                if (record->rx_open_timing_valid &&
                    record->observed_rx_slot == record->completed_rx_slot) {
                    update_node_latency(&node_open_to_rmarker[src_idx],
                        dwt_high32_delta_to_us(record->rx_open_high32,
                                               record->rx_ts_high32));
                }
            }
        }
...
int brrs_init(void)
{
    ...
    {
#if BRRS_OPT_RX_ERROR_DIAG
        final_log_info("EXP4_RX_ERROR_DIAG_CONFIG_CSV,enabled=1,version=1,status_bytes=6,queue_capacity=32,sample_capacity=64,scope=error_timeout_only,raw_scope=pre_post_48bit,legacy_counter_scope=post_rearm_four_bit,slot_identity=estimate_only,status=PASS");
#endif
    ...
    exp4_error_detail_post_rearm_stats.min_us = 0xFFFFFFFF;
    ...
    exp4_event_to_metadata_stats.min_us = 0xFFFFFFFF;
    ...
    while (1)
    {
...
#if BRRS_EXPERIMENT == 4
                {
                    ...
                    bool schedule_pass =
                        (exp4_sync_tx_delayed_late == 0 &&
                         ...
                         exp4_event_to_metadata_stats.count ==
                             exp4_rx_good_events &&
                         exp4_event_to_status_clear_stats.count ==
                             exp4_rx_good_events &&
                         exp4_event_to_header_stats.count ==
                             exp4_rx_good_events &&
...
#endif
#if BRRS_OPT_RX_ERROR_DIAG
                         && exp4_error_diag.queue_overflow == 0U
                         && exp4_error_diag.queue_count == 0U
                         && exp4_error_diag.processed == exp4_error_diag.events
                         && exp4_error_diag.events == total_rx_errors + total_rx_timeouts
                         && exp4_error_diag.hw_faults == 0U
#endif
                         );
                    ...
                    snprintf(s, sizeof(s),
                             "TDMA validation: wrong-length=%lu wrong-slot=%lu wrong-superframe=%lu data-config-errors=%lu rx-schedule-late=%lu sync-delayed-late=%lu rearm-deadline-miss=%lu rx-buffer-overrun=%lu deferred-overflow=%lu rx-timeout=%lu rx-error=%lu",
                             ...
                             (unsigned long)exp4_rearm_deadline_misses,
                             (unsigned long)exp4_rx_buffer_overruns,
                             (unsigned long)exp4_rx_record_overflows,
...

                    {
                        ...
                        snprintf(s, sizeof(s),
                                 "EXP4_DOUBLE_BUFFER_CSV,mode=manual,release=%s,rx_good_events=%lu,rdb_good_events=%lu,rdb_dispatches=%lu,rdb_host_mismatch=%lu,rdb_incomplete=%lu,rdb_incomplete_recovered=%lu,rdb_global_ciadone=%lu,rdb_resync=%lu,free_count=%lu,free_min_us=%lu,free_max_us=%lu,free_avg_x1000_us=%llu,overrun=%lu",
                                 ...
                                 (unsigned long)exp4_rdb_global_ciadone_fallbacks,
                    ...
                    {
                        latency_stats_t *cumulative_stats[] = {
                            &exp4_event_to_metadata_stats,
                            &exp4_event_to_status_clear_stats,
                            &exp4_event_to_header_stats
                        };
                        const char *cumulative_names[] = {
                        ...
                        for (phase = 0U; phase < 3U; phase++) {
                            latency_stats_t *stats = cumulative_stats[phase];
                            uint64_t avg_x1000 = stats->count ?
                                (stats->sum_us * 1000ULL / stats->count) : 0U;
                            snprintf(s, sizeof(s),
                                     "EXP4_HOT_PATH_PHASE_CSV,phase=%s,count=%lu,min_us=%lu,max_us=%lu,avg_x1000_us=%llu",
                                     cumulative_names[phase],
                                     (unsigned long)stats->count,
                                     (unsigned long)(stats->count ?
                                         stats->min_us : 0U),
                                     (unsigned long)stats->max_us,
                    ...
                    {
                        uint64_t hot_avg_x1000 = exp4_hot_path_stats.count ?
                            (exp4_hot_path_stats.sum_us * 1000ULL /
                             exp4_hot_path_stats.count) : 0U;
                        snprintf(s, sizeof(s),
                                 "EXP4_HOT_PATH_CSV,scope=event_detect_to_buffer_free,count=%lu,min_us=%lu,max_us=%lu,avg_x1000_us=%llu,p50_us=%lu,p90_us=%lu,p95_us=%lu,p99_us=%lu,p999_us=%lu,hist_overflow=%lu",
                                 (unsigned long)exp4_hot_path_stats.count,
                                 (unsigned long)(exp4_hot_path_stats.count ?
                                     exp4_hot_path_stats.min_us : 0U),
                                 (unsigned long)exp4_hot_path_stats.max_us,
                                 (unsigned long long)hot_avg_x1000,
                                 (unsigned long)exp4_hot_path_percentile(50U, 100U),
                                 (unsigned long)exp4_hot_path_percentile(90U, 100U),
                                 (unsigned long)exp4_hot_path_percentile(95U, 100U),
                                 (unsigned long)exp4_hot_path_percentile(99U, 100U),
                                 (unsigned long)exp4_hot_path_percentile(999U, 1000U),
                                 (unsigned long)exp4_hot_path_hist_overflow);
                        final_log_info(s);
                    }

                    snprintf(s, sizeof(s),
                             "EXP4_DEFERRED_CSV,batches=%lu,pending=%u,queue_overflow=%lu,rearm_deadline_miss=%lu,rx_timeout=%lu,rx_error=%lu,status=%s",
                             (unsigned long)exp4_deferred_batches,
                             (unsigned int)exp4_rx_record_count,
                             (unsigned long)exp4_rx_record_overflows,
                             (unsigned long)exp4_rearm_deadline_misses,
                             (unsigned long)total_rx_timeouts,
                             (unsigned long)total_rx_errors,
                    ...
                    snprintf(s, sizeof(s),
                             "EXP4_SFDTO_TRIAL_CSV,requested=%u,readback=%u,reads=%lu,default_formula=%u,status=%s",
                             (unsigned)EXP4_TRIAL_DATA_SFD_TIMEOUT,
                             (unsigned)exp4_trial_sfd_readback,
                             (unsigned long)exp4_trial_sfd_readback_count,
                             (unsigned)(PREAMBLE_SYMBOLS + 1 + SFD_SYMBOLS - DATA_PAC_SYMBOLS),
                             (exp4_trial_sfd_readback_count == 1U &&
                              exp4_trial_sfd_readback == EXP4_TRIAL_DATA_SFD_TIMEOUT) ? "PASS" : "FAIL");
                    final_log_info(s);
...
#else
                /* Exp4's optional fast-switch wrapper is not defined in the
                 * single-link experiments. Retain their full configure path. */
                if (dwt_configure(&config_data) == DWT_SUCCESS) {
#endif
                    config_is_sync = false;
                    /* Read once before the first RX is armed. No extra SPI
                     * or logging in the per-slot reception hot path. */
                    if (exp4_trial_sfd_readback_count == 0U) {
                        uint8_t sfd_bytes[2];
                        exp4_spi_read_device(DTUNE0_ID, 2U, 2U, sfd_bytes);
                        exp4_trial_sfd_readback = (uint16_t)sfd_bytes[0] |
                            ((uint16_t)sfd_bytes[1] << 8);
                        exp4_trial_sfd_readback_count++;
                    }
#if ENABLE_CIR
                    enable_cir_diagnostics();
#else
        ...
        /* ========== [D] RX 폴링 ========== */
        {
            ...
            if ((exp4_fint_status & FINT_STAT_RXTO_BIT_MASK) != 0U) {
                status_reg |= DWT_INT_RXFTO_BIT_MASK;
            }
            if (exp4_data_burst_active &&
#if BRRS_EXP4_IRQ_PENDING
                exp4_event_detected &&
            ...
            if (status_reg & DWT_INT_RXFCG_BIT_MASK) {
                ...
                bool exp4_record_storable =
                    exp4_rx_record_count < BRRS_MAX_DATA_SLOTS;
                exp4_rx_record_t overflow_record = {0};
                exp4_rx_record_t *exp4_record = exp4_record_storable ?
                    &exp4_rx_records[exp4_rx_record_count] : &overflow_record;
                ...
                /* RXFCG can precede per-buffer CIADONE. Re-arming at that
                 * point can interrupt adjusted-timestamp production, whether
                 * RXFCG was found by polling or delivered through GPIO IRQ.
                 * Every comparison mode therefore validates readiness first
                 * and re-arms immediately after the matching buffer is ready. */
                ...
                if ((exp4_rdb_status & exp4_rdb_current_ready_mask) !=
                     ...
                     * timeout for a bit that cannot arrive. This bounded wait
                     * occurs before RX re-arm and therefore remains visible
                     * in the measured guard requirement. */
                    uint32_t retry_start_cycles = dwt_timer_get_cycles();
                    uint32_t retry_elapsed_us = 0U;
                    bool cia_error = false;
                    bool global_ciadone_ready = false;
                    for (;;) {
                        uint32_t cia_status;
                        ...
                        cia_status = dwt_readsysstatuslo();
                        if (cia_status & DWT_INT_CIAERR_BIT_MASK) {
                            cia_error = true;
                            dwt_writesysstatuslo(DWT_INT_CIAERR_BIT_MASK);
                            break;
                        }
                        /* No later frame can begin until this loop succeeds
                         * and RX is re-armed. Therefore a
                         * global CIADONE observed after this buffer's RXFCG
                         * is unambiguously the completion of this frame even
                         * when the matching RDB CIADONEx mirror is absent. */
                        if (cia_status & DWT_INT_CIADONE_BIT_MASK) {
                            global_ciadone_ready = true;
                            break;
                        }
                        retry_elapsed_us =
                    ...
                    if ((exp4_rdb_status & exp4_rdb_current_ready_mask) ==
                            exp4_rdb_current_ready_mask ||
                        global_ciadone_ready) {
                        exp4_rdb_incomplete_recovered++;
                        if (global_ciadone_ready) {
                            exp4_rdb_global_ciadone_fallbacks++;
                        }
                    } else {
                        if (cia_error) {
                ...
                update_node_latency(&exp4_event_to_header_stats,
                    exp4_cycles_to_us_ceil(dwt_timer_get_cycles() -
                                            exp4_hot_path_start_cycles));
                ...
                exp4_record->header_length = (uint8_t)header_read_len;
                exp4_record->completed_rx_slot = completed_rx_slot;
                exp4_record->observed_rx_slot = observed_rx_slot;
                ...
                if (exp4_record_storable) {
                    ...
                    exp4_advance_after_event(observed_slot_valid ?
                        observed_rx_slot : completed_rx_slot, true);
                } else {
                    exp4_rx_record_overflows++;
                    exp4_close_data_burst(EXP4_BURST_CLOSE_DEADLINE);
                }
...
#if BRRS_OPT_RX_ERROR_DIAG
                brrs_rxerr_record_t error_record;
                exp4_error_diag_pre(&error_record, exp4_fint_status, true);
                error_record.rearmed = exp4_rearm_after_event(exp4_event_start_cycles, false);
                error_record.rearm_cycles = dwt_timer_get_cycles() - exp4_event_start_cycles;
#else
                exp4_rearm_after_event(exp4_event_start_cycles, false);
#endif
                failed_rx_slot = exp4_slot_from_event_time();
#if BRRS_OPT_RX_ERROR_DIAG
                status_reg = exp4_error_diag_post(&error_record, failed_rx_slot,
                    exp4_rearm_needed ? &exp4_error_detail_post_rearm_stats : NULL);
#else
...
#if BRRS_OPT_RX_ERROR_DIAG
                brrs_rxerr_record_t error_record;
                exp4_error_diag_pre(&error_record, exp4_fint_status, false);
                error_record.rearmed = exp4_rearm_after_event(exp4_event_start_cycles, false);
                error_record.rearm_cycles = dwt_timer_get_cycles() - exp4_event_start_cycles;
#else
                exp4_rearm_after_event(exp4_event_start_cycles, false);
