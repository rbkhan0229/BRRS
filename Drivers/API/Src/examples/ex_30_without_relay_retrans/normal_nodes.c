...
#if defined(TEST_WITHOUT_RELAY_AND_RETRANS_NORMAL)
...
// Message queues - retrans_queue removed for ablation study
...
/* NO RETRANSMISSION - Ablation study mode */
#if TEST_MODE
...
static bool is_expected_ack_slot_for(uint8_t ack_slot_idx, uint8_t tx_slot_idx) {
    ...
    // Don't expect ACK from relay slots
...
int plain_schedule_normal_node(void)
{
    ...
    while (1)
    {
        ...
        if (last_sync_cycles != 0 && !config_is_sync) {
            ...
            if (dwt_timer_elapsed(last_sync_cycles, config_switch_cycles)) {
                ...
                if (pre_switch_status & DWT_INT_RXFCG_BIT_MASK) {
                    ...
                    // Process if it's DATA or RELAY_DATA in odd period
            ...
            if (sync_valid && time_advanced && slot_time_reached && slot_not_executed) {
                ...
                if (is_data_period) {
                    ...
                    if (current_period_in_cycle == 1) {
                        ...
                        tx_type_desc = "DATA TX (Period 1 only)";
                ...
                snprintf(tx_decision_msg, sizeof(tx_decision_msg),
                        "[%ums] TX Decision: %s - Period: %d/%d",
                        get_elapsed_ms(last_sync_cycles), tx_type_desc, current_period_in_cycle, PERIODS_PER_CYCLE);
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK) {  // RX frame received
            ...
            if ((current_period_in_cycle % 2 == 1) &&
                ...
                /* No filtering: All nodes (including FL/FR) accept DATA from all other nodes */
                /* This allows FL/FR to behave identically to normal nodes in ablation study */
                        ...
                        if (!cumulative_ack_confirmed[src_slot_idx]) {
                            ...
                            if (cumulative_ack_count >= TEST_EXPECTED_ACKS) {
                                ...
                                if (!success_in_current_cycle) {
                                    ...
                                    /* NO RETRANSMISSION - state update removed */
