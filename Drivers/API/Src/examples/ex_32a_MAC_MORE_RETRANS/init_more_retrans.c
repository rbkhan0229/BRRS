 ...
 *           - Period 1,3,5,7: DATA 전송, Period 2,4,6,8: ACK_ARRAY 전송
...
#if defined(TEST_INIT_MORE_RETRANS)
...
#define PERIODS_PER_CYCLE   8       // 8 periods per cycle (4 DATA/ACK pairs)
...
#define MSG_TYPE_ACK_ARRAY    0x07  // ACK array broadcast (Period 2,4,6,8)
...
enum {
  ...
  IDX_PERIOD_INFO = 6,  // 6: Period number (1-8) - SYNC only, reuses ORIG_SRC position
...
static uint8_t period_in_cycle = 1;  // 1-8 (4 period pairs)

/* ========== Pair 단위 통계 (사이클당 4개 pair) ========== */
// Pair 1 (Period 1-2)
...
// Pair 4 (Period 7-8)
static uint32_t pair4_data_success = 0, pair4_data_fail = 0, pair4_data_idle = 0;
static uint32_t pair4_fl_relay_success = 0, pair4_fl_relay_fail = 0, pair4_fl_relay_idle = 0;
static uint32_t pair4_fr_relay_success = 0, pair4_fr_relay_fail = 0, pair4_fr_relay_idle = 0;
 ...
 */
int dwt_init_more_retrans(void)
{
    ...
    while (1)
    {
         ...
         *
         * Period 구조:
         *   - 홀수 Period (1,3,5,7): DATA 전송/수신
         *   - 짝수 Period (2,4,6,8): ACK_ARRAY 전송/수신
         */
            ...
            if (last_sync_cycles == 0 || dwt_timer_elapsed(last_sync_cycles, period_interval_cycles)) {
                ...
                if (current_cycle > 1000 && !final_stats_printed) {
                    ...
                    snprintf(final_stats, sizeof(final_stats),
                            ...
                            pair4_data_success, pair4_data_fail, pair4_data_idle,
                            pair4_fl_relay_success, pair4_fl_relay_fail, pair4_fl_relay_idle,
                            pair4_fr_relay_success, pair4_fr_relay_fail, pair4_fr_relay_idle,
                            total_cycles,
                ...
                tx_msg[IDX_PERIOD_INFO] = period_in_cycle;  // Add period info (1-8) for recovery
        ...
        if (last_sync_cycles != 0) {
            ...
            if (should_execute_slot) {
                 ...
                 *
                 * Period 홀수 (1,3,5,7): DATA 전송
                 *   - Period 1: 새 메시지 생성 + retrans_msg에 저장
                 *   - Period 3,5,7: retrans_msg에서 복사 (재전송)
                 *   - 이미 성공 시 IDLE (TX 스킵)
                 *
                 * Period 짝수 (2,4,6,8): ACK_ARRAY 전송
                 *   - data_received_from[] 배열을 페이로드로 전송
                 */
                if (active_slot_type == SLOT_TYPE_OWN) {
                    /* SEQ 1 - Determine period type: odd (1,3,5,7) = DATA, even (2,4,6,8) = ACK_ARRAY */
                    ...
                    if (is_data_period) {
                        ...
                        if (tx_state == TX_STATE_IDLE || data_success_in_current_cycle) {
                            ...
                            /* Period 3, 5, 7 should retransmit the SAME message from Period 1 */
                            if (period_in_cycle == 1) {
                                ...
                                /* Store for potential retransmission in Period 3, 5, 7 */
                                memcpy(retrans_msg, tx_msg, sizeof(tx_msg));
                            } else {
                                /* Period 3, 5, 7: RETRANSMIT same message from Period 1 */
         ...
         *
         * 수신 메시지 처리:
         *   - Period 홀수 (1,3,5,7): DATA/RELAY_DATA → data_received_from[] 업데이트
         *   - Period 짝수 (2,4,6,8): ACK_ARRAY → ACK 확인 및 성공 판정
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK) {  // RX frame received
            ...
            /*
             * [D-2] Period 홀수 (1,3,5,7): DATA 메시지 수신 처리
             *
            ...
            /*
             * [D-3] Period 짝수 (2,4,6,8): ACK_ARRAY 메시지 수신 처리
             *
                            ...
                            if (cumulative_ack_count >= TEST_EXPECTED_ACKS) {
                                ...
                                if (!data_success_in_current_cycle) {
                                    ...
                                    if (period_in_cycle == 1 || period_in_cycle == 2) {
                                        ...
                                        pair3_data_success++;
                                    } else if (period_in_cycle == 7 || period_in_cycle == 8) {
                                        pair4_data_success++;
                                    }
