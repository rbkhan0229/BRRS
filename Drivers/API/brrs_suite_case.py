...
def side(a):
    ...
    try:
        ...
        for job, proc in zip(jobs,workers):
            ...
            if c['conditions'].get('profile')=='paper':
                expected.update(suite_profile='paper',suite_block=str(c['conditions']['run']),
...
def main():
    ...
    p.add_argument('--profile',choices=['preparation','paper'],default='preparation')
