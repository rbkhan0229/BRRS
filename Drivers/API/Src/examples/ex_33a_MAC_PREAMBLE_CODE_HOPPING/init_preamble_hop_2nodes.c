...
#if defined(TEST_INIT_PREAMBLE_HOP_2NODES)

extern void test_run_info(unsigned char *data);

/* Example application name */
#define APP_NAME "INITIATOR NODE Preamble hopping v1.0 (2-NODE TEST: INIT+N5)"
...
#if TEST_MODE
#define TEST_TOTAL_NODES 2     // Only 2 nodes: INIT + NODE_5
#define TEST_EXPECTED_ACKS 1   // Expect ACK from NODE_5 only
#endif
...
/* Global Period Counter for Preamble Hopping (16-bit) */
...
enum {
  ...
  IDX_VEHICLE_ID_HI = 10,     // SYNC only: CHANNEL_ID high byte
  IDX_VEHICLE_ID_LO = 11      // SYNC only: CHANNEL_ID low byte
};

/* INIT Node has 3 transmission slots: SEQ 1 (own), SEQ 10 (FL relay), SEQ 12 (FR relay) */
typedef enum {
    SLOT_TYPE_OWN = 1       // SEQ 1 - Own data transmission (2-node: no relay)
} init_slot_type_t;
...
static bool is_expected_ack_slot_for(uint8_t ack_slot_idx, uint8_t tx_slot_idx) {
    ...
    if (tx_slot_idx == SLOT_IDX_INIT) {
        return (ack_slot_idx == SLOT_IDX_NODE_5);
    }
    return false;
    #else
 ...
 */
int dwt_init_preamble_hop(void)
{
    ...
    dwt_setrxtimeout(0); // 2-node: disable RX timeout to avoid dead zone when only N5 transmits
    ...
    while (1)
    {
        ...
        if (last_sync_cycles == 0 || dwt_timer_elapsed(last_sync_cycles, period_interval_cycles)) {
            ...
            if (last_sync_cycles == 0 || dwt_timer_elapsed(last_sync_cycles, period_interval_cycles)) {
                ...
                /* Get preamble codes: current for this period, next for Normal's 17.2ms switch */
                ...
                /* Add Preamble Codes to SYNC message (replaces Global Counter) */
                tx_msg[IDX_CURRENT_PREAMBLE] = current_preamble;  // Current period DATA/ACK
                tx_msg[IDX_NEXT_PREAMBLE] = next_preamble;        // Next period SYNC reception

                /* Add Vehicle ID to SYNC for interference rejection */
                tx_msg[IDX_VEHICLE_ID_HI] = (uint8_t)(CHANNEL_ID >> 8);
                tx_msg[IDX_VEHICLE_ID_LO] = (uint8_t)(CHANNEL_ID & 0xFF);

                /* Apply current preamble code to both configs BEFORE configure */
                ...
                /* Switch to SYNC config before SYNC TX (with current preamble code) */
                dwt_forcetrxoff();
                if (dwt_configure(&config_sync) == DWT_SUCCESS) {
                    static char sync_config_log[120];
                    snprintf(sync_config_log, sizeof(sync_config_log),
                            "Switched to SYNC config (PLEN512) cur=%d, next=%d", current_preamble, next_preamble);
                    test_run_info((unsigned char *)sync_config_log);
                ...
                /* Increment global counter for next period */
                global_period_counter++;

                /* Print SYNC frame content with preamble info */
                snprintf(sync_debug, sizeof(sync_debug), "SYNC TX: GC=%u, cur=%d, next=%d, P%d",
                        global_period_counter - 1, current_preamble, next_preamble, period_in_cycle);
                            ...
                            if (cumulative_ack_count >= TEST_EXPECTED_ACKS) {
                                ...
                                if (!data_success_in_current_cycle) {
                                    ...
                                    snprintf(init_success_msg, sizeof(init_success_msg),
                                            "DATA SUCCESS in Period %d (Pair %d) - ACK received from N5",
