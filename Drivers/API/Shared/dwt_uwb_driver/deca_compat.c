...
/*
read rx data multiple times from buffer
*/
void dwt_readrxdata_in_chunks(uint8_t *dest_buffer, uint16_t total_len, uint16_t chunk_size)
{
    uint16_t offset = 0;
    ...
    while (offset < total_len)
    {
        ...
        // 실제 read 호출: dest_buffer에 offset만큼 이동하여 저장
        dwt_readrxdata(dest_buffer + offset, len, offset);
