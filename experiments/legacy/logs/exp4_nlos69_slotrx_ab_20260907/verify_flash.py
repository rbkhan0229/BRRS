...
ap=argparse.ArgumentParser();ap.add_argument('--variant',choices=['A','B'],required=True);ap.add_argument('--side',choices=['init','tx'],required=True);a=ap.parse_args()
m=json.loads((root/'manifest.json').read_text());rs=m['variants'][a.variant];roles=['init'] if a.side=='init' else ['N2','N3','N4']
