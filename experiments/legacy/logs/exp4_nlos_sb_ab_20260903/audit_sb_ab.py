#!/usr/bin/env python3
"""Read-only audit of fixed-role SB A/B captures with SP=2002 and full configure. Never converts PER FAIL to PASS."""
import csv
import hashlib
...
def audit(mode, run):
    fast = False
    sb = int(mode)
    folder = ROOT.parent / f"exp4_nlos_sbab_full_6.9m_g200_l15_pac8_sb{sb}_sp2002_20260903_spiopt"
    ...
    try:
        for role, serial in SERIALS.items():
            ...
            assert meta["environment"] == "nlos_sbab_full"
            for key, value in {"preamble_symbols":"32", "sensor_count":"3", "guard_us":"200", "lead_us":"15", "pac":"8", "sync_buffer_us":str(sb), "sync_prep_us":"2002", "target_cycles":"1000", "rx_event_source":"fint-polling", "phy_fast_skip_pgf":"disabled", "phy_config_profile":"disabled", "rx_path_profile":"disabled", "spim_start_end_profile":"disabled"}.items():
            ...
            if role != "init":
                v.verify_sensor(lines, 32, 3, int(role[1]), 200, sb, 2002, expected_cycles=1000, phy_fast=fast)
        ...
        v.verify_init(logs["init"], 32, 3, 200, 15, 8, sb, 2002, 5.0, spi_opt=True, expected_cycles=1000, phy_fast=fast)
...
if __name__ == "__main__":
    plan = [(m,r) for r in (1,4,7) for m in ("2000","1703")]
    if len(sys.argv) == 3:
        mode, run = sys.argv[1], int(sys.argv[2])
        assert (mode, run) in plan
        plan = [(mode,run)]
    elif len(sys.argv) != 1:
        raise SystemExit("usage: audit_sb_ab.py [2000|1703 1|4|7]")
