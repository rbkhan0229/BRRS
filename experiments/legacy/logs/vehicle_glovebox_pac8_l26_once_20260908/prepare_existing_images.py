...
old=BASE/'logs/nlos69_lead_screen_rotation_20260907_2326/paper_exp4_m32_pac8_l26_k13_s6_b02'
prior=checked(old)
m=json.loads((BASE/'logs/vehicle_onepass_run_20260908_0039/vehicle_manifest_TEMPLATE.json').read_text())
m['environment']='VEHICLE_GLOVEBOX_ENGINE_ON_20260908'
m['boards']['init']['location']='glovebox'
...
m['setup_record']={'recorded_at':datetime.now().astimezone().isoformat(),
 'vehicle_model':None,'vehicle_type':None,'vehicle_model_status':'not provided',
 'actual_rx_location':'glovebox; planned future frunk is not the measured location',
 'installation_mapping':'user installed in previously documented physical node order',
 'engine_state':'on, confirmed by user','hub_supply':'external power connected, confirmed by user',
 'doors':'all closed, confirmed by user','windows':'not separately specified',
 'hood_trunk':'not separately specified beyond user statement all doors closed',
 'user_setup_quote':'시동 걸려있고 허브 전원 연결했고 모든 문 다 닫혀있어',
 'physical_changes_by_agent':False,'distance_m':None,
 'scope':'one M32 PAC8 lead26 S6 K13 run, 1000 superframes, fixed original roles',
 'comparison_limit':'prior lead26 zero-loss K13 run used rotated roles in NLOS; vehicle keeps fixed installation roles'}
...
for j in c['jobs']:
 ...
 j.update(hex=prev['hex'],hex_sha256=prev['hex_sha256'],elf_sha256=prev['elf_sha256'],rtt_address=prev['rtt_address'],
          script=Path(j['argv'][1]).name,args=j['argv'][2:]+['--no-build','--timeout','180'])
