...
#if defined(TEST_WITHOUT_RELAY_AND_RETRANS_INIT)

extern void test_run_info(unsigned char *data);

/* Example application name */
#define APP_NAME "INITIATOR NODE ABLATION STUDY - NO RELAY, NO RETRANS"
...
/* Per-node latency tracking (10 nodes only - no relay slots) */
static latency_stats_t node_latency[TOTAL_ARRAY_SIZE] = {
    ...
    {0xFFFFFFFF, 0, 0, 0},  // (unused slot 9)
    {0xFFFFFFFF, 0, 0, 0},  // FR (slot 10)
    {0xFFFFFFFF, 0, 0, 0}   // (unused slot 11)
};
...
/* Slot index mapping for 10-node system (no relay) */
...
static uint32_t failed_cycles = 0;               // All pairs failed
...
/* No retransmission or relay - single transmission in Period 1 only */
...
static uint8_t get_slot_index(uint8_t msg_type, uint8_t src_node, uint8_t orig_src) {
    if (msg_type == MSG_TYPE_DATA) {
        ...
        // FL and FR have specific slots (8 and 10)
        if (src_node == NODE_FL) {
...
/*
 * Check if we expect ACK from a specific slot
 *
 * Parameters:
 *   ack_slot_idx: Slot index that sends ACK
 *   tx_slot_idx:  Slot index that transmitted (unused for INIT - always own data)
 *
 * Returns:
 *   true:  Expect ACK from ack_slot_idx
 *   false: Do not expect ACK from ack_slot_idx
 */
...
int plain_schedule(void)
{
    ...
    /* INIT node has 1 slot only (no relay) */
    ...
    /* Slot timer for SEQ 1 only (absolute timing from SYNC TX completion) */
    ...
    while (1)
    {
        ...
        if (last_sync_cycles == 0 || dwt_timer_elapsed(last_sync_cycles, period_interval_cycles)) {
            ...
            if (last_sync_cycles == 0 || dwt_timer_elapsed(last_sync_cycles, period_interval_cycles)) {
                ...
                if ((period_count - 1) % PERIODS_PER_CYCLE == 0) {
                    ...
                    /* NO RETRANSMISSION - state reset removed */
                ...
                /* Check if we've completed 3 cycles and print final statistics */
                //MARK: 종료 조건
                if (current_cycle > 1000 && !final_stats_printed) {
                    /* Print final statistics - ABLATION STUDY (NO RELAY, NO RETRANS) */
                    ...
                    snprintf(final_stats, sizeof(final_stats),
                            "\n=== PAIR STATISTICS (1000 cycles) - ABLATION STUDY ===\n"
        ...
        /* 3. Check for INIT node's 1 slot: SEQ 1 (own data only, no relay) */
        //MARK: INIT 1-slot handling: SEQ 1 only
        if (last_sync_cycles != 0) {
            ...
            if (should_execute_slot) {
                ...
                if (active_slot_type == SLOT_TYPE_OWN) {
                    ...
                    if (is_data_period) {
                        /* NO RETRANSMISSION: Only send DATA in Period 1 */
                        if (period_in_cycle == 1) {
                            /* Period 1: Send NEW DATA frame */
                            should_transmit = true;
                            tx_type_desc = "OWN DATA TX (Period 1 only)";
                            ...
                            /* Period 3, 5: NO RETRANSMISSION - Skip transmission */
                            should_transmit = false;
                            tx_type_desc = "DATA SKIP (NO RETRANS - ablation study)";

                            /* Count as IDLE by pair */
                            if (period_in_cycle == 3) {
                                pair2_data_idle++;
                ...
                snprintf(tx_decision_msg, sizeof(tx_decision_msg),
                        "TX Decision: %s - SlotType: %d, Period: %d/%d",
                        tx_type_desc, active_slot_type, period_in_cycle, PERIODS_PER_CYCLE);
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK) {  // RX frame received
            ...
            /* Period odd (1,3,5): Track DATA messages (no relay) */
            if ((period_count % 2 == 1) && (rx_buffer[IDX_MSG_TYPE] == MSG_TYPE_DATA)) {
                ...
                /* NO RELAY - FL/FR data storage removed */
            }
            ...
            /* Period even (2,4,6): Process ACK_ARRAY messages */
                ...
                /* INIT has 1 slot to check: INIT(0) - no relay */
                ...
                /* NO RELAY - FL/FR relay ACK processing removed */
            }
