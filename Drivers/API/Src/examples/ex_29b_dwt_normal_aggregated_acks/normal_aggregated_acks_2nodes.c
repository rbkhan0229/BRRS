...
#if defined(TEST_NORMAL_AGGREGATED_ACKS_2NODES)
...
//MARK: TEST MODE 설정
    ...
    #define TEST_EXPECTED_ACKS 1   // Only expect ACK from INIT
    ...
    //#define TEST_NODE_FL       // FL node, slot 9 (SEQ 9: 80ms-90ms) - needs relay via INIT (disabled for 2-node test)
    //#define TEST_NODE_FR       // FR node, slot 11 (SEQ 11: 100ms-110ms) - needs relay via INIT (disabled for 2-node test)
...
static bool is_expected_ack_slot_for(uint8_t ack_slot_idx, uint8_t tx_slot_idx) {
    ...
    // 2-node: only expect ACK from INIT
    return (ack_slot_idx == SLOT_IDX_INIT);
    #else
 ...
 *   [F] RX 타임아웃/에러 처리
 */
int dwt_normal_aggregated_acks(void)
{
    ...
    while (1)
    {
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK) {
            ...
            if (rx_buffer[IDX_MSG_TYPE] == MSG_TYPE_SYNC) {
                /* Validate vehicle identity - reject SYNC from different vehicle */
                uint16_t received_vehicle_id = ((uint16_t)rx_buffer[IDX_VEHICLE_ID_HI] << 8)
                                              | rx_buffer[IDX_VEHICLE_ID_LO];
                if (received_vehicle_id != CHANNEL_ID) {
                    test_run_info((unsigned char *)"SYNC rejected: different vehicle");
                    dwt_forcetrxoff();
                    dwt_rxenable(DWT_START_RX_IMMEDIATE);
                    continue;
                }
            ...
            if ((current_period_in_cycle % 2 == 1) &&
                ...
                #if TEST_MODE
                /* 2-node test: ignore RELAY_DATA and direct DATA from FL/FR */
