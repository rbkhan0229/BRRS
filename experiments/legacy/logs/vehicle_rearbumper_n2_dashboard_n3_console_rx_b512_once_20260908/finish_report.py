...
prior = json.loads((R.parent / 'vehicle_ceiling_light_b512_n2flipped_once_20260908/RESULTS.json').read_text())
...
result = {'verdict': 'PASS_PER_NODE' if passed else 'FAIL', 'rf_runs_started': 1,
          ...
          'limitations': ['New N2 orientation after mounting on rear bumper was not specified.',
                         'Multiple node positions changed; one run does not isolate an individual cause.',
...
lines = ['# 차량 N2 뒷범퍼 · N3 대시보드 · RX 수납함 위 · 비컨 M512 1회', '',
...
lines += ['', '정확히 1회 RF 실행. 종료 후 7대 플래시 readback PASS 및 정지를 확인했다. 로컬·원격 잔여 캡처 프로세스 0, 기존 Git 브랜치·HEAD·dirty 상태 변경 없음. commit/push 없음. N2·N3·RX 위치를 함께 바꾼 1회 관측이므로 각 위치의 효과를 따로 확정하지 않는다.', '',
