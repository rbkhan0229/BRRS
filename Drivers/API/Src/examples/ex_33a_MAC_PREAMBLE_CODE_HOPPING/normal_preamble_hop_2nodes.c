...
#if defined(TEST_NORMAL_PREAMBLE_HOP_2NODES)
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK) {
            ...
            if (rx_buffer[IDX_MSG_TYPE] == MSG_TYPE_SYNC) {
                ...
                if (received_vehicle_id != CHANNEL_ID) {
                    static char reject_msg[80];
                    snprintf(reject_msg, sizeof(reject_msg),
                            "SYNC rejected: vehicle_id=0x%04X (expected 0x%04X)",
                            received_vehicle_id, CHANNEL_ID);
                    test_run_info((unsigned char *)reject_msg);
                    dwt_forcetrxoff();
                    dwt_rxenable(DWT_START_RX_IMMEDIATE);
