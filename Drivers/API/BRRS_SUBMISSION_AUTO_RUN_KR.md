...
| `exp3` | A/B/C, 조건당 1회 | 3 |
| `exp4` | M=32/64/128/256, 조건당 1회 | 4 |

위 표는 저수준 일괄 실행기의 한 PAC·한 슬롯 설정 기준이다. 현재 차량 준비용 manifest는 Exp4의4종 M × PAC4/8 × 6슬롯/계산상 최대 슬롯, 총16조건을 생성한다. 최대 부하 실패 시 중간 슬롯을 낮춰 검사하는 조건부 탐색과 양쪽 case 실행 경로는 [차량 설정 안내](BRRS_VEHICLE_MANIFEST_KR.md)를 따른다. 논문용 반복·회전은 [새 case 실행 경로](BRRS_PAPER_CAMPAIGN_KR.md)에 연결됐다. 이 저수준 런너의 기본값과 구분한다.
