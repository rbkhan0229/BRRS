...
r=Path(__file__).resolve().parent;x=json.loads((r/'RESULTS.json').read_text());m=json.loads((r/'manifest.json').read_text());runs=x['primary_runs'];p=x['pooled']
a=p['A']['N3']['per_pct'];b=p['B']['N3']['per_pct'];reduction=(a-b)/a*100 if a else 0
text=f'''# NLOS 6.9m · M32 burst / 슬롯별 scheduled-RX A/B

**N3 합산 PER: A {a:.2f}% → B {b:.2f}% (상대 감소 {reduction:.1f}%).** B의 전체 합산 PER은0.967%지만 N3가2.90%여서 각 노드 PER<1% 목표는 미달이다.
...
for i,s in enumerate(runs,1):
    n=s['nodes'];t=s['total'];text+=f"| {i}. {s['tag']} | {n['N2']['rx']} / {n['N2']['per_pct']:.2f}% | {n['N3']['rx']} / {n['N3']['per_pct']:.2f}% | {n['N4']['rx']} / {n['N4']['per_pct']:.2f}% | {t['rx']}/3000 / {t['per_pct']:.3f}% | {s['goal']} |\n"
text+='\n| 방식·2회 합산 | N2 | N3 | N4 |\n|---|---:|---:|---:|\n'
for variant in ['A','B']:
    text+=f"| {variant} | "+' | '.join(f"{p[variant][node]['rx']}/2000 / {p[variant][node]['per_pct']:.2f}%" for node in ['N2','N3','N4'])+' |\n'
text+='\n실행 시각은 호스트 캡처·검증 구간(KST)이다. 각 RF 실험 자체는1000SF≈10초다.\n\n'
for s in runs:text+=f"- {s['tag']}: {s['started_at']} – {s['finished_at']}\n"
text+='''
## 오류 카운터와 검증
...
'''
for s in runs:
    e=s['errors'];vals=[str(e[k]) for k in ['sfd_timeout','phr','crc','rxfsl']]
    if s['variant']=='A' and e['fint_only']>0:vals=[str(e[k])+'*' for k in ['sfd_timeout','phr','crc','rxfsl']]
    text+=f"| {s['tag']} | {e['rx_errors']} | "+' | '.join(vals)+f" | {e['fint_only']} | {e['fwto']}/{e['pto']} |\n"
text+='''
...
'''
for s in runs:
    for w in s['window_records']:
        terminal=sum(int(w[k]) for k in ['rx_good','timeout','error'])
        text+=f"| {s['tag']} / N{w['owner']} | {w['attempted']}/{w['armed']} | {w['late']} | {w['rx_good']} | {w['timeout']} | {w['error']} | {terminal} |\n"
text+='''
## 해석
...
|---|---|---|
'''
for variant,role,label in [('A','init','INIT A'),('B','init','INIT B'),('A','N2','N2 공통'),('A','N3','N3 공통'),('A','N4','N4 공통')]:
    d=m['variants'][variant][role];text+=f"| {label} | {d['serial']} | `{d['sha256']}` |\n"
text+='''
