...
static uint8_t tx_poll_msg[120] = { 0x41, 0x88, 0, 0xCA, 0xDE, 'W', 'A', 'V', 'E', 0xE0, 0, 0, [13 ... 119]=0x01 };
    ...
    while (1)
    {
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                received_packet_nb=rx_buffer[ALL_MSG_SN_IDX];
                rx_buffer[ALL_MSG_SN_IDX] = 0;
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    uint32_t resp_rx_ts, poll_rx_ts, resp_tx_ts;
                    ...
                    sprintf(str_to_print,"%d is received", received_packet_nb);
                    test_run_info((unsigned char *)str_to_print);
                    sprintf(str_to_print,"tof: %.12f s", tof);
                    test_run_info((unsigned char *)str_to_print);
                    //END
                    total_send++;
                    retransmission=false;
                    if(total_send >= 100000)
                    {
                      test_run_info("END!!");
                      break;
                    }
        ...
        else if(status_reg & SYS_STATUS_ALL_RX_TO)
        {
            ...
            test_run_info("PACKET IS TO");
            retransmission=true;
        }
        else
        {
          dwt_writesysstatuslo(SYS_STATUS_ALL_RX_ERR);
          test_run_info("PACKET IS ERR");
            retransmission=true;
        }
 ...
 *    6.8M data rate used (around 400 us).
