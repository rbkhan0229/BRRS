...
def make_campaign(a):
    ...
    if a.dry_run:
        print(json.dumps({'rf_execution_performed':False,'builds_performed':False,'case_ids':[c['id'] for c in cases]},indent=2));return
    ...
    spec={'manifest':str(a.manifest.resolve()),'manifest_sha256':sha(a.manifest),'profile':a.profile,'stage':a.stage,
          'confirmation':a.confirmation,'capacity_candidates':a.capacity_candidates,'case_ids':[c['id'] for c in cases]}
...
def run_campaign(a):
    ...
    report={'actions':actions,'groups':aggregate([planned[x] for x in spec['case_ids']],observations),'rf_execution_performed':not a.dry_run and any(x['action']=='DEPLOY_THEN_RUN' for x in actions)}
