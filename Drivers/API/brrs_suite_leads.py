#!/usr/bin/env python3
"""Stage0 grid -> candidate/neighbours -> confirmed PAC-specific lead manifest."""
import argparse
import copy
import json
...
def candidates(m,report):
    ...
    for pac in m['pacs']:
        ...
        eligible=[l for l in points if all(n in points and points[n]<1 for n in [l-1,l,l+1])]
        if not eligible:raise ValueError(f'PAC{pac}: no three-adjacent-lead region with PER<1%; do not freeze')
        selected[str(pac)]=min(eligible,key=lambda l:(max(points[l-1],points[l],points[l+1]),sum(points[n] for n in [l-1,l,l+1]),abs(l-20),l))
    return selected

def freeze(m,report):
    if report['rejected'] or any(g['status']!='PASS' for g in report['groups']):
        raise ValueError('every candidate/neighbour confirmation must be complete and each run PER<1%')
...
def main():
    ...
    if a.action=='candidates':
        ...
        result['lead_selection']={'frozen':True,'lead_us_by_pac':values,'evidence':{'method':'complete candidate and adjacent-lead confirmations; every run PER<1%; pooled Wilson95 upper<1%',
