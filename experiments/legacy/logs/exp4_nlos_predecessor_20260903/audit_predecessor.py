#!/usr/bin/env python3
"""Read-only audit of M32 234/432 owner-order crossover; preserve PER failures."""
import csv
import hashlib
...
def base_name(preamble, run):
    return f"exp4_{preamble}_s3_r{run}"
...
def audit(mode, run):
    ...
    sb = 2000
    preamble = 32
    sequence = "432" if mode == "432" else None
    seq_suffix = "_seq432" if sequence else ""
    matches = sorted(ROOT.parent.glob(f"exp4_nlos_predecessor_full_6.9m_g200_l15_pac8_sb2000_sp2002{seq_suffix}_????????_spiopt/{base_name(preamble, run)}_init.log"))
    ...
    try:
        for role, serial in SERIALS.items():
            ...
            assert meta["environment"] == "nlos_predecessor_full"
            ...
            if role != "init":
                v.verify_sensor(lines, preamble, 3, int(role[1]), 200, sb, 2002, sequence=sequence, expected_cycles=1000, phy_fast=fast)
        ...
        v.verify_init(logs["init"], preamble, 3, 200, 15, 8, sb, 2002, 5.0, sequence=sequence, spi_opt=True, expected_cycles=1000, phy_fast=fast)
...
if __name__ == "__main__":
    plan = [("234",1), ("432",1), ("234",4)]
    if len(sys.argv) == 3:
        mode, run = sys.argv[1], int(sys.argv[2])
        assert (mode, run) in plan
        plan = [(mode,run)]
    elif len(sys.argv) != 1:
        raise SystemExit("usage: audit_predecessor.py [234|432 1|4]")
