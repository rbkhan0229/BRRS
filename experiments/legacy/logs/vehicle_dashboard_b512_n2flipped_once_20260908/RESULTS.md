# 차량 대시보드 RX · 비컨 M512 · N2 뒤집은 뒤 1회

**노드별 PER <1% 목표: FAIL. RX 1,000SF 완료: 예.**

사용자는 N2만 뒤집었다고 알렸다. 회전축은 지정하지 않았다. 나머지 배치·시동 켜짐·유전원 허브·문 닫힘은 직전 확인 상태를 이어 기록했다. RX 대시보드, N2/N3 앞범퍼 바깥, N4 운전석, N5 조수석, N6/N7 트렁크.
...
|---|---:|---:|---:|---:|---:|
| N2 앞범퍼 A (뒤집음) | 1 | 2 / 2 | 0 / 2,000 | 100.00% | 100.00% |
| N3 앞범퍼 B | 1 | 3 / 3 | 0 / 3,000 | 100.00% | 100.00% |
| N4 운전석 | 1,000 | 2,000 / 2,000 | 1,977 / 2,000 | 1.15% | 9.65% |
| N5 조수석 | 1,000 | 2,000 / 2,000 | 1,756 / 2,000 | 12.20% | 5.70% |
| N6 트렁크 A | 1,000 | 2,000 / 2,000 | 1,698 / 2,000 | 15.10% | 2.70% |
| N7 트렁크 B | 1,000 | 2,000 / 2,000 | 1,907 / 2,000 | 4.65% | 27.40% |
...
전체 RX 7,338/13,000, 예정 전송 기준 손실률 43.5538%. 실제 TX 완료 8,005회.

## 수집 및 오류

- N2: metadata=FAIL, END 통계=True, END 비컨=False, delayed-TX late=0; SYNC loss: 1 timeouts  RX errors=25  beacon_config_errors=0  data_config_errors=0
- N3: metadata=FAIL, END 통계=True, END 비컨=False, delayed-TX late=0; SYNC loss: 1 timeouts  RX errors=11  beacon_config_errors=0  data_config_errors=0
...
```text
RX timeouts=5000 (fwto=5000 pto=0)  RX errors=662 (sfdto=650 phe=5 fce=0 fsl=7 fint-only=0 overrun=0)  delayed schedule late=0  data config errors=0
TDMA validation: wrong-length=0 wrong-slot=0 wrong-superframe=0 data-config-errors=0 rx-schedule-late=0 sync-delayed-late=0 rearm-deadline-miss=0 rx-buffer-overrun=0 deferred-overflow=0 rx-timeout=5000 rx-error=662
EXP4_DOUBLE_BUFFER_CSV,mode=manual,release=rdb_w1c_plus_cmd_db_toggle,rx_good_events=7338,rdb_good_events=7338,rdb_dispatches=7338,rdb_host_mismatch=0,rdb_incomplete=0,rdb_incomplete_recovered=0,rdb_global_ciadone=0,rdb_resync=0,free_count=7338,free_min_us=38,free_max_us=38,free_avg_x1000_us=38000,overrun=0
...
EXP4_DEFERRED_CSV,batches=1000,pending=0,queue_overflow=0,rearm_deadline_miss=0,rx_timeout=5000,rx_error=662,status=PASS
EXP4_SPI_CSV,mode=persistent_data_burst,begin=1000,end=1000,active=0,begin_fail=0,end_fail=0,device_id_fail=0,state_error=0,transfer_error=0,direct_xfers=239633,direct_timeout=0,recovery=0,status=PASS
EXP4_STATUS_CSV,schedule=PASS,timing=PASS,collection=PASS,link=LOSS
EXP4_SUMMARY_CSV,32,6,13,1,26,16,347,250,13,1000,13000,7338,435538,93926,166400,9999998,0,0,PASS
```
...
정확히 1회 RF 실행. 종료 후 7대 플래시 readback PASS 및 정지를 확인했다. 로컬·원격 잔여 캡처 프로세스 0, 기존 Git 브랜치·HEAD·dirty 상태 변경 없음. commit/push 없음. 설치 방향 변경 1회 관측으로 단일 원인을 확정하지 않는다.
