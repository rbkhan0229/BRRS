...
def main():
    ...
    roles=['N2','N3','N4','N5','N6','N7'] if args.side=='tx' else ['init']
    ...
    # Quiesce all explicitly assigned boards before any begins listening.
    # New boards may contain another role's firmware from an earlier experiment.
    for role in roles:
        jl=pylink.JLink()
        try:
            jl.open(serial_no=int(records[role]['serial']))
            jl.set_tif(pylink.enums.JLinkInterfaces.SWD)
            jl.connect('NRF52840_XXAA',speed=4000)
            jl.halt()
            if not jl.halted(): raise RuntimeError(f'{role} could not halt for setup')
        finally:
            jl.close()
    target.mkdir(parents=True,exist_ok=False)
    state={'variant':args.variant,'run':args.run,'side':args.side,'started_at':now(),
        'status':'STARTING','setup':'All assigned side boards halted before first capture starts','parameters':manifest['parameters_by_variant'][args.variant],'workers':{},
