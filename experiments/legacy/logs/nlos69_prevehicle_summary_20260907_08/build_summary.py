#!/usr/bin/env python3
"""Consolidate saved assessments; no hardware access or source-data edits."""
import hashlib
import json
...
ROOT = OUT.parents[1]
POST = ROOT / 'logs/vehicle_onepass_run_20260908_0039/postprocess_20260908'
LEDGER = POST / 'revalidated_assessments.json'
OBS = POST.parent / 'observations.json'
assessments = json.loads(LEDGER.read_text())
observations = json.loads(OBS.read_text())
stages = ['stage0', 'exp1', 'exp2', 'exp3', 'exp4', 'exp5']
stage_counts = {
    s: dict(Counter(v['verdict'] for v in assessments.values()
                    if v['conditions']['stage'] == s)) for s in stages
}
tx6 = [v for v in observations.values() if v['conditions'].get('sensors') == 6]
assert len(assessments) == 86 and len(observations) == 40 and len(tx6) == 16
assert all(v['verdict'] == 'PASS' and v['worst_node_per_percent'] < 1 for v in observations.values())
assert sum(v['aggregate']['offered'] for v in tx6) == 132000
assert sum(v['aggregate']['rx'] for v in tx6) == 131999
...
for v in tx6:
    c = v['conditions']
    matrix.setdefault((c['preamble'], len(c['slot_owners'])), {})[c['rx_pac']] = v
matrix_rows = [[m, k, f"{x[4]['worst_node_per_percent']:g}",
                f"{x[8]['worst_node_per_percent']:g}"]
               for (m, k), x in sorted(matrix.items())]
inventory = []
for bundle, v in assessments.items():
    c = v['conditions']
    inventory.append({
        'bundle': bundle, 'case_id': v['case_id'], 'stage': c['stage'],
        'conditions': c, 'verdict': v['verdict'],
        'worst_node_per_percent': v['worst_node_per_percent'],
        'nodes_by_serial': v['nodes_by_serial'],
        'source_assessment_ledger': str(LEDGER),
    })
inventory.sort(key=lambda x: (stages.index(x['stage']), x['case_id'], x['bundle']))
summary = {
    'scope': 'NLOS 6.9 m prevehicle campaign, 2026-09-07 evening to 2026-09-08 early morning KST',
    'unique_actual_rf_bundles': len(assessments),
    'stage_verdict_counts': stage_counts,
    'final_new_rf_conditions': len(observations),
    'final_tx6_conditions': len(tx6),
    'final_tx6_offered': 132000, 'final_tx6_rx': 131999,
    'omitted_unmeasured_small_tx_conditions': 28,
    'source_sha256': {str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in [LEDGER, OBS]},
    'inventory': inventory,
}
(OUT / 'SUMMARY.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n')

doc = '''# 차량 실험 직전 NLOS 6.9m 실험 항목 정리
...
for x in inventory:
    c = x['conditions']
    case_link = f"[{x['case_id']}]({link(x['bundle'])})"
    inventory_rows.append([c['stage'], case_link, c['preamble'], c['rx_pac'],
        c['lead_us'], len(x['nodes_by_serial']), len(c['slot_owners']),
        f"{x['worst_node_per_percent']:.4f}".rstrip('0').rstrip('.'), x['verdict']])
doc = doc.replace('__MATRIX__', rows_table(
    ['M', '슬롯/SF', 'PAC4 / lead27µs 최악 PER(%)', 'PAC8 / lead26µs 최악 PER(%)'], matrix_rows))
doc = doc.replace('__INVENTORY__', rows_table(
    ['단계', '실행', 'M', 'PAC', 'lead µs', 'TX', 'K', '최악 노드 PER(%)', '판정'], inventory_rows))
(OUT / 'SUMMARY.md').write_text(doc)
print(json.dumps({'output': str(OUT / 'SUMMARY.md'), 'rf_count': len(inventory),
                  'stage_verdicts': stage_counts, 'tx6_conditions': len(tx6)}, ensure_ascii=False))
