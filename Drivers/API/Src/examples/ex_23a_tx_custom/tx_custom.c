/*! ----------------------------------------------------------------------------
 *  @file    tx_custom.c
 *  @brief   TX Custom Preamble Code Sequence (17->24->9)
 *
 * @author Decawave
 *
...
#if defined(TEST_TX_CUSTOM)

extern void test_run_info(unsigned char *data);

/* Example application name */
#define APP_NAME "TX CUSTOM SEQUENCE v1.0"
...
/* TX message for custom sequence communication */
...
/* Custom preamble code sequence: 17->18->19->20->21->22->23->24->9->10->11->12->13->14->15->16 (then repeat) */
...
static uint32_t packets_per_code = 500; // Send 500 packets per preamble code
...
int tx_custom(void)
{
    ...
    /* Initialize custom preamble code transmission */
    current_tx_index = 0;
    ...
    test_run_info((unsigned char *)"Starting custom sequence: 17->24->9->16->17...");
    
    /* Loop forever sending frames periodically. */
    while (1)
    {
        /* Use custom sequence preamble code for this transmission */
        uint8_t current_code = preamble_codes_custom[current_tx_index];
        ...
        if (total_send > 0 && (total_send % packets_per_code) == 0) {
            ...
            char switch_msg[100];
            uint8_t next_code = preamble_codes_custom[current_tx_index];
            sprintf(switch_msg, "TX switching to code: %d (after %lu pkts)", 
                    next_code, packets_per_code);
        ...
        if(total_send >= 10000)
        {
            ...
            test_run_info((unsigned char *)"=== Custom Sequence Usage Stats ===");
