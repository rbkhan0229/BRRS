/*! ----------------------------------------------------------------------------
 *  @file    normal_more_retrans.c
 *  @brief   Normal Node for UWB MAC Protocol V2 - Aggregated ACKs
 *
 *           TDMA-based UWB protocol with aggregated acknowledgments.
 *           Normal nodes transmit DATA in odd periods (1,3,5,7) and ACK_ARRAY in even periods (2,4,6,8).
...
#if defined(TEST_NORMAL_MORE_RETRANS)
...
#define PERIODS_PER_CYCLE   8       // 8 periods per cycle (4 period pairs)
...
static uint32_t pair4_success = 0, pair4_fail = 0, pair4_idle = 0;
...
typedef struct {
    ...
    uint32_t period_skip_count[8];     // Skip count per period (index 0-7 for P1-P8)
} sync_loss_stats_t;
...
#define USE_TX_POWER_INDEX  TX_POWER_INDEX_0dB    // <-- 여기서 변경
 ...
 *   - 21.2ms 주기, 12-슬롯 구조
 *   - Period 홀수(1,3,5,7): DATA 전송/수신
 *   - Period 짝수(2,4,6,8): ACK_ARRAY 전송/수신
 *
 * Normal 노드 동작:
 ...
 *   [F] RX 타임아웃/에러 처리
 */
int dwt_normal_more_retrans(void)
{
    ...
    while (1)
    {
        ...
        if (last_sync_cycles != 0 && !final_stats_printed) {
            ...
            if (dwt_timer_elapsed(last_sync_cycles, sync_timeout_cycles)) {
                ...
                snprintf(final_stats, sizeof(final_stats),
                        ...
                        "Pair 4 (Period 7-8): Success=%d, Fail=%d, IDLE=%d\n\n"
                        ...
                        pair4_success, pair4_fail, pair4_idle,
                        total_cycles,
                ...
                static char sync_loss_output[550];
                ...
                snprintf(sync_loss_output, sizeof(sync_loss_output),
                        ...
                        sync_loss_stats.period_skip_count[6],
                        sync_loss_stats.period_skip_count[7]);
         ...
         *   [D-2] Period 홀수(1,3,5,7): DATA TX
         *   [D-3] Period 짝수(2,4,6,8): ACK_ARRAY TX
         */
         ...
         *   [E-2] Period 홀수(1,3,5,7) DATA 처리 (data_received_from 업데이트)
         *   [E-3] Period 짝수(2,4,6,8) ACK_ARRAY 처리 (ACK 확인)
         */
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK) {
            ...
            /*
             * ----- [E-2] Period 홀수(1,3,5,7): DATA 수신 처리 -----
             *
            ...
            /*
             * ----- [E-3] Period 짝수(2,4,6,8): ACK_ARRAY 수신 처리 -----
             *
                        ...
                        if (!cumulative_ack_confirmed[src_slot_idx]) {
                            ...
                            if (cumulative_ack_count >= TEST_EXPECTED_ACKS) {
                                ...
                                if (!success_in_current_cycle) {
                                    ...
                                    } else if (current_period_in_cycle == 8) {
                                        pair4_success++;  // Period 8 evaluates Pair 4
                                    }
